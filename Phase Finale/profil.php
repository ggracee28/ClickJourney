<?php
    ...
    $purchased_voyages_details = [];
    if (!empty($user['voyages_achetes']) && is_array($user['voyages_achetes'])) {
        foreach ($user['voyages_achetes'] as $achat) {
            $voyage = getVoyageById((int)$achat['voyage_id']);
            if ($voyage) {
                $purchased_voyages_details[] = [
                    'voyage' => $voyage,
                    'montant' => $achat['montant'],
                    'date_achat' => $achat['date_achat'],
                    'lien_voyage' => $achat['lien_voyage'],
                    'options_details' => $achat['options_details'] ?? []
                ];
            }
        }
    }
...
<header class="header">
    <div class="nav-container">
            ...
            <div class="profile-container">
                ...
                <div class="profile-field" style="flex-direction: column; align-items: flex-start; border-bottom: none;">
                    <div style="display: flex; width: 100%;">
                        ...
                        <div class="field-value" style="flex: 2;">
                            <?php if (!empty($purchased_voyages_details)): ?>
                                <ul class="voyages-list">
                                    <?php foreach ($purchased_voyages_details as $achat_details): ?>
                                        <li>
                                            <a href="<?php echo htmlspecialchars($achat_details['lien_voyage']); ?>" target="_blank">
                                                <?php echo htmlspecialchars($achat_details['voyage']['titre']); ?>
                                            </a>
                                            (<?php echo number_format($achat_details['montant'], 2); ?>€) - <?php echo htmlspecialchars($achat_details['date_achat']); ?>
                                            <?php if (!empty($achat_details['options_details'])): ?>
                                                <div class="options-summary">
                                                    <strong>Options:</strong>
                                                    <?php foreach ($achat_details['options_details'] as $etape_id => $etape): ?>
                                                        <p style="margin-left: 10px;"><em><?php echo htmlspecialchars($etape['titre']); ?>:</em>
                                                        <?php if (isset($etape['options']['hebergement'])): ?>
                                                            <span>Hébergement: <?php echo htmlspecialchars($etape['options']['hebergement']['description']); ?></span><br>
                                                        <?php endif; ?>
                                                        <?php if (isset($etape['options']['restauration'])): ?>
                                                            <span>Restauration: <?php echo htmlspecialchars($etape['options']['restauration']['description']); ?></span><br>
                                                        <?php endif; ?>
                                                        <?php if (isset($etape['options']['activites'])): ?>
                                                            <?php 
                                                                ...
                                                                echo "<span>Activités: " . implode(', ', $activites_descriptions) . "</span><br>";
                                                            ?>
                                                        <?php endif; ?>
                                                        <?php if (isset($etape['options']['transport'])): ?>
                                                            <span>Transport: <?php echo htmlspecialchars($etape['options']['transport']['description']); ?></span><br>
                                                        <?php endif; ?>
                                                        </p>
                                                    <?php endforeach; ?>
                                                </div>
        ...
        <script src="profil-edit.js"></script>
    </body>
