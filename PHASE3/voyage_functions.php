...
/**
 * Ajoute un voyage au panier de l'utilisateur
 * @param int $userId ID de l'utilisateur
 * @param int $voyageId ID du voyage à ajouter
 * @param float $prixTotal Prix total du voyage avec options
 * @return bool True si mis à jour avec succès
 */
function addVoyageToPanier($userId, $voyageId, $prixTotal = null) {
    ...
    foreach ($users as &$user) {
        if ($user['id'] == $userId) {
            ...
            // Stocker le prix total si disponible
            if ($prixTotal !== null) {
                if (!isset($user['voyages_prix'])) {
                    $user['voyages_prix'] = [];
                }
                $user['voyages_prix'][$voyageIdStr] = $prixTotal;
            }
...
/**
 * Cette fonction n'est plus utilisée pour ajouter au panier, mais juste pour le tracking
 * des consultations si nécessaire
 */
function addConsultedVoyage($userId, $voyageId) {
    // Cette fonction reste inchangée pour compatibilité avec le reste du site
    // mais on ne l'utilise plus pour ajouter au panier
    return true;
}
